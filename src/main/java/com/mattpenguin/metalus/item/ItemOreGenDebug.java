package com.mattpenguin.metalus.item;

import com.mattpenguin.metalus.Metalus;
import com.mattpenguin.metalus.block.MetalusBlocks;
import com.mattpenguin.metalus.block.ModBlocks;
import com.mattpenguin.metalus.common.Constant;
import com.mattpenguin.metalus.common.IDebugable;
import com.mattpenguin.metalus.net.OreDebugPacket;
import com.mattpenguin.metalus.net.PacketHandler;
import net.minecraft.block.BlockState;
import net.minecraft.entity.player.PlayerEntity;
import net.minecraft.item.ItemStack;
import net.minecraft.item.ItemUseContext;
import net.minecraft.util.ActionResult;
import net.minecraft.util.ActionResultType;
import net.minecraft.util.Hand;
import net.minecraft.util.math.BlockPos;
import net.minecraft.util.math.MathHelper;
import net.minecraft.world.IWorld;
import net.minecraft.world.World;
import net.minecraft.world.gen.ChunkGenerator;
import net.minecraft.world.gen.GenerationSettings;
import net.minecraft.world.gen.Heightmap;
import net.minecraft.world.gen.feature.OreFeatureConfig;

import javax.annotation.Nonnull;
import java.util.BitSet;
import java.util.Random;

public class ItemOreGenDebug extends MetalusItem {
    public ItemOreGenDebug(String name) {
        super(name);
    }

    @Override
    @Nonnull
    public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand) {
        if (world.isRemote) {
            PacketHandler.sendToServer(new OreDebugPacket());
        }
        return new ActionResult<>(ActionResultType.PASS, player.getHeldItem(hand));
    }


    public static boolean generate(IWorld world, Random rand, BlockPos pos, int size) {
        int x = Math.round(pos.getX());
        int y = Math.round(pos.getY());
        int z = Math.round(pos.getZ());
        int adj = Math.round(size/2.0F);
        int xMin = x - adj;
        int xMax = x + adj;
        int yMin = y - adj;
        int yMax = y + adj;
        int zMin = z - adj;
        int zMax = z + adj;

        int xDif = Math.abs(xMax - xMin);
        int yDif = Math.abs(yMax - yMin);
        int zDif = Math.abs(zMax - zMin);

        BlockPos.MutableBlockPos genPos = new BlockPos.MutableBlockPos();


        for (int i = -xDif; i < xDif; ++i) {
            float iNorm = (float)i/size;
            for (int j = -yDif; j < yDif; ++j) {
                float jNorm = (float)j/size;
                for (int k = -zDif; k < zDif; ++k) {
                    float kNorm = (float)k/size;
                    if ((iNorm*iNorm + jNorm*jNorm + kNorm*kNorm) + (0.5F + rand.nextFloat()) <= 1.0F) {
                        genPos.setPos(pos.getX() + i, pos.getY() + j, pos.getZ() + k);
                        world.setBlockState(genPos, MetalusBlocks.Ores.copperOre.getDefaultState(), 2);
                    }
                }
            }
        }
        return true;
    }



    public static boolean place(IWorld worldIn, Random rand, BlockPos pos, int config) {
        float f = rand.nextFloat() * (float) Math.PI;
        float f1 = (float) config / 4.0F;
        int i = MathHelper.ceil(((float) config / 16.0F * 2.0F + 1.0F) / 2.0F);
        double d0 = (float) pos.getX() + MathHelper.sin(f) * f1;
        double d1 = (float) pos.getX() - MathHelper.sin(f) * f1;
        double d2 = (float) pos.getZ() + MathHelper.cos(f) * f1;
        double d3 = (float) pos.getZ() - MathHelper.cos(f) * f1;
        int j = 2;
        double d4 = pos.getY() + rand.nextInt(4) - 2;
        double d5 = pos.getY() + rand.nextInt(4) - 2;
        int k = pos.getX() - MathHelper.ceil(f1) - i;
        int l = pos.getY() - 2 - i;
        int i1 = pos.getZ() - MathHelper.ceil(f1) - i;
        int j1 = 2 * (MathHelper.ceil(f1) + i);
        int k1 = 2 * (2 + i);

        for (int l1 = k; l1 <= k + j1; ++l1) {
            for (int i2 = i1; i2 <= i1 + j1; ++i2) {
                return func_207803_a(worldIn, rand, config, d0, d1, d2, d3, d4, d5, k, l, i1, j1, k1);
            }
        }

        return false;
    }

    protected static boolean func_207803_a(IWorld worldIn, Random random, int config, double p_207803_4_, double p_207803_6_, double p_207803_8_, double p_207803_10_, double p_207803_12_, double p_207803_14_, int p_207803_16_, int p_207803_17_, int p_207803_18_, int p_207803_19_, int p_207803_20_) {
        int i = 0;
        BitSet bitset = new BitSet(p_207803_19_ * p_207803_20_ * p_207803_19_);
        BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
        double[] adouble = new double[config * 4];

        for (int j = 0; j < config; ++j) {
            float f = (float) j / (float) config;
            double d0 = MathHelper.lerp(f, p_207803_4_, p_207803_6_);
            double d2 = MathHelper.lerp(f, p_207803_12_, p_207803_14_);
            double d4 = MathHelper.lerp(f, p_207803_8_, p_207803_10_);
            double d6 = random.nextDouble() * (double) config / 16.0D;
            double d7 = ((double) (MathHelper.sin((float) Math.PI * f) + 1.0F) * d6 + 1.0D) / 2.0D;
            adouble[j * 4 + 0] = d0;
            adouble[j * 4 + 1] = d2;
            adouble[j * 4 + 2] = d4;
            adouble[j * 4 + 3] = d7;
        }

        for (int l2 = 0; l2 < config - 1; ++l2) {
            if (!(adouble[l2 * 4 + 3] <= 0.0D)) {
                for (int j3 = l2 + 1; j3 < config; ++j3) {
                    if (!(adouble[j3 * 4 + 3] <= 0.0D)) {
                        double d12 = adouble[l2 * 4 + 0] - adouble[j3 * 4 + 0];
                        double d13 = adouble[l2 * 4 + 1] - adouble[j3 * 4 + 1];
                        double d14 = adouble[l2 * 4 + 2] - adouble[j3 * 4 + 2];
                        double d15 = adouble[l2 * 4 + 3] - adouble[j3 * 4 + 3];
                        if (d15 * d15 > d12 * d12 + d13 * d13 + d14 * d14) {
                            if (d15 > 0.0D) {
                                adouble[j3 * 4 + 3] = -1.0D;
                            } else {
                                adouble[l2 * 4 + 3] = -1.0D;
                            }
                        }
                    }
                }
            }
        }

        for (int i3 = 0; i3 < config; ++i3) {
            double d11 = adouble[i3 * 4 + 3];
            if (!(d11 < 0.0D)) {
                double d1 = adouble[i3 * 4 + 0];
                double d3 = adouble[i3 * 4 + 1];
                double d5 = adouble[i3 * 4 + 2];
                int k = Math.max(MathHelper.floor(d1 - d11), p_207803_16_);
                int k3 = Math.max(MathHelper.floor(d3 - d11), p_207803_17_);
                int l = Math.max(MathHelper.floor(d5 - d11), p_207803_18_);
                int i1 = Math.max(MathHelper.floor(d1 + d11), k);
                int j1 = Math.max(MathHelper.floor(d3 + d11), k3);
                int k1 = Math.max(MathHelper.floor(d5 + d11), l);

                for (int l1 = k; l1 <= i1; ++l1) {
                    double d8 = ((double) l1 + 0.5D - d1) / d11;
                    if (d8 * d8 < 1.0D) {
                        for (int i2 = k3; i2 <= j1; ++i2) {
                            double d9 = ((double) i2 + 0.5D - d3) / d11;
                            if (d8 * d8 + d9 * d9 < 1.0D) {
                                for (int j2 = l; j2 <= k1; ++j2) {
                                    double d10 = ((double) j2 + 0.5D - d5) / d11;
                                    if (d8 * d8 + d9 * d9 + d10 * d10 < 1.0D) {
                                        int k2 = l1 - p_207803_16_ + (i2 - p_207803_17_) * p_207803_19_ + (j2 - p_207803_18_) * p_207803_19_ * p_207803_20_;
                                        if (!bitset.get(k2)) {
                                            bitset.set(k2);
                                            blockpos$mutableblockpos.setPos(l1, i2, j2);
                                            worldIn.setBlockState(blockpos$mutableblockpos, MetalusBlocks.Ores.copperOre.getDefaultState(), 2);
                                            ++i;
                                        }
                                    }
                                }
                            }
                        }
                    }
                }
            }
        }

        return i > 0;
    }

}
